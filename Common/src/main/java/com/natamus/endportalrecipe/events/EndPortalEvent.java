...
public class EndPortalEvent {
	...
	public static boolean onLeftClick(Level level, Player player, BlockPos cpos, Direction direction) {
		...
		if (Services.TOOLFUNCTIONS.isPickaxe(hand)) {
			if (ConfigHandler.mustHaveSilkTouchToBreakPortal) {
				if (EnchantmentHelper.getItemEnchantmentLevel(level.registryAccess().registryOrThrow(Registries.ENCHANTMENT).getHolderOrThrow(Enchantments.SILK_TOUCH), hand) < 1) {
					return true;
				}
			}

			BlockState cbs = level.getBlockState(cpos);
			if (cbs.getBlock().equals(Blocks.END_PORTAL_FRAME)) {
				...
				if (ConfigHandler.addBrokenPortalFramesToInventory) {
					...
					ItemEntity frame = new ItemEntity(level, cpos.getX(), cpos.getY()+1, cpos.getZ(), portalframe);
					level.addFreshEntity(frame);
					
					if (BlockFunctions.isFilledPortalFrame(cbs)) {
						ItemEntity eoe = new ItemEntity(level, cpos.getX(), cpos.getY()+1, cpos.getZ(), endereye);
						level.addFreshEntity(eoe);
					}
				}
				
				level.destroyBlock(cpos, false);
				...
				while (it.hasNext()) {
					BlockPos np = it.next();
					if (level.getBlockState(np).getBlock().equals(Blocks.END_PORTAL)) {
